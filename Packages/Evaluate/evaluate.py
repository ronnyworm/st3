...
class EvaluateCommand(sublime_plugin.TextCommand):
    ...
    def replace(self, edit, thread, offset):
        ...
        prefix = original
        try:
            main = original + " = " + str(round(result, 4))
        except Exception as e:
            main = original + " = " + str(result)
...
class EvaluateCall(threading.Thread):
    ...
    def run(self):
        ...
        if self.original.lstrip().startswith('!'):
            ...
            try:
                to_evaluate = self.original.lstrip()[1:]

                tmp_global = {
                    "math": math,
                    "datetime": datetime
                }
                code = compile(to_evaluate, '<string>', 'eval')
...
#                pass
            ...
            try:
                to_evaluate = self.original.replace(",", ".")
                to_evaluate = re.sub(r'[A-Za-z]+', '', to_evaluate)
